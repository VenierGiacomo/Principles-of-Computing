...
class Puzzle:
    ...
    def __init__(self, puzzle_height, puzzle_width, initial_grid=None):
        ...
        self.grid = [[col + puzzle_width * row
    ...
    def clone(self):
        ...
        Returns a Puzzle object
        """
        new_puzzle = Puzzle(self.height, self.width, self.grid)
        return new_puzzle
    ...
    def current_position(self, solved_row, solved_col):
        ...
        """
        solved_value = (solved_col + self.width * solved_row)

        for row in range(self.height):
            for col in range(self.width):
                if self.grid[row][col] == solved_value:
                    return (row, col)
    ...
    def update_puzzle(self, move_string):
        ...
        for direction in move_string:
            if direction == "l":
                ...
                self.grid[zero_row][zero_col] = self.grid[zero_row][zero_col - 1]
                self.grid[zero_row][zero_col - 1] = 0
                zero_col -= 1
            elif direction == "r":
                assert zero_col < self.width - 1, "move off grid: " + direction
                self.grid[zero_row][zero_col] = self.grid[zero_row][zero_col + 1]
                self.grid[zero_row][zero_col + 1] = 0
                zero_col += 1
            elif direction == "u":
                assert zero_row > 0, "move off grid: " + direction
                self.grid[zero_row][zero_col] = self.grid[zero_row - 1][zero_col]
                self.grid[zero_row - 1][zero_col] = 0
                zero_row -= 1
            elif direction == "d":
                assert zero_row < self.height - 1, "move off grid: " + direction
                self.grid[zero_row][zero_col] = self.grid[zero_row + 1][zero_col]
                self.grid[zero_row + 1][zero_col] = 0
                zero_row += 1
            else:
