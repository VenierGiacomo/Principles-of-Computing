...
class ClickerState:
    ...
    def __str__(self):
        ...
        '''
        return 'Total amount: ' + str('{0:.1f}'.format(self._tot_of_cookies)) + '\n' \
               'Current amount: ' + str('{0:.1f}'.format(self._current_cookies)) + '\n' \
    ...
    def wait(self, time):
        ...
        if time <= 0:
            ...
            self._tot_of_cookies += time * self._current_cps
...
def simulate_clicker(build_info, duration, strategy):
    ...
    clo_build_info = build_info.clone()
    new_click = ClickerState()

    while 0 <= duration:
        item = strategy(new_click.get_cookies(), new_click.get_cps(), 
                        duration, clo_build_info)
        if item is None:
            # no resources anymore, no more items will be purchased
            break
        item_cost_of_item = clo_build_info.get_cost(item)
        wait_time = new_click.time_until(item_cost_of_item)
        if duration < wait_time:
            ...
            new_click.buy_item(item, item_cost_of_item, clo_build_info.get_cps(item))
            clo_build_info.update_item(item)
...
def strategy_cheap(cookies, cps, time_left, build_info):
    '''
    always select the cheap item that you can afford in the time left;
    ...
    cheap = min(inventory.values())
    # still enough time left to buy at least one more item
    if cheap <= (cookies + (time_left * cps)):
        # looking in the dictionary for a key to known minimal (cheap) value
...
def strategy_best(cookies, cps, time_left, build_info):
    ...
    for item in xrange(len(cost_sorted_list) - 1):
        item1 = cost_sorted_list[item][1]
        current_cost1 = build_info.get_cost(item1)
        item2 = cost_sorted_list[item + 1][1]
        current_cost2 = build_info.get_cost(item2)
        # still enough time to buy at least one from a pair of considered items
        if current_cost1 or current_cost2 <= resource:
            a = (current_cost1 / cps) + current_cost2 / (build_info.get_cps(item1) + cps)
            b = (current_cost2 / cps) + current_cost1 / (build_info.get_cps(item2) + cps)
            if fast_from_zero < a and b:
